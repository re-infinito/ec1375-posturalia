...
- **Copiar por trozos** (el archivo nunca pasa por el navegador): el navegador del equipo llama a `zoom-copiar-trozo` con el número de trozo; el servidor pide a Zoom ese rango (`Range`, 50 MB) y lo sube a la **carga por trozos de Nextcloud** (`/remote.php/dav/uploads/…`, con `Destination`); el último trozo hace `MOVE` al destino. 50 MB < 100 MB (límite de Cloudflare) y cada llamada queda muy por debajo del tiempo máximo de la función. El avance se guarda en `evaluaciones.video.migracion` para reanudar. El destino lo arma el servidor y se valida con `rutaNasPermitida` + que esté bajo `Portafolios/*/03-Evaluacion/` y termine en `.mp4`.
