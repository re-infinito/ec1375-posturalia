...
2. **Biblioteca sin modo claro ("formato de día")** — pendiente, sin tocar. `ruta-estudio.html`/`ruta-alineacion.html` (modo rail del shell) se mantienen deliberadamente siempre oscuras (ver "Shell CRM del candidato" arriba, decisión de la MISMA tarde) porque el contenido de las diapositivas usa colores fijos optimizados para fondo oscuro — meterle modo claro no es solo un toggle, implica revisar contraste en las 121+57 diapositivas. Reportado por Fernando independientemente de esa decisión; falta que Diego confirme si de verdad quiere invertirla.
3. **Biblioteca "le faltan completar" pantallas** — investigado, no hay huecos: las 121 diapositivas de `META` tienen contenido real 1:1 (sin placeholders). Lo que SÍ falta: las 7 pantallas nuevas del Módulo 7 de Alineación (`ruta-alineacion.html`, `vp-1`...`vp-7`) no están indexadas en la Biblioteca (`sidIndiceBiblioteca: null`) — sin tocar, falta decidir con Diego si se agregan ahí o si aplica mejor el backlog #18 (Biblioteca como visor independiente).
