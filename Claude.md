...
**⚠️ Hallazgo pendiente de resolver con Diego (encontrado 15 sep, sin tocar):** `plantilla_IEC_blanco.pdf` **no está en blanco** — es el IEC real y ya lleno de Humberto (su nombre y el de su evaluadora aparecen en las 83 páginas, columna "NOMBRE DEL CANDIDATO"/firma). Cualquier expediente generado con el script hoy le mete los datos reales de Humberto a la sección del IEC de CUALQUIER candidato. Hace falta que Diego consiga/genere una plantilla realmente en blanco antes de generar el expediente de un candidato real. `_internal_no_publicar/03-documentos-referencia/FORMATO PORTAFOLIO-1375-2026 copia.pdf` (45 págs., blanco, ver siguiente párrafo) **no resuelve esto** — su sección de IEC son solo 2 páginas separadoras ("IEC" / "PRODUCTOS"), no el instrumento completo de 83 páginas.

**15 sep — segunda ronda de validación contra `FORMATO PORTAFOLIO-1375-2026 copia.pdf`:** Diego señaló este archivo (blanco, con marcador de página por cada documento) como más autoritativo que el expediente real de Humberto — el nombre sugiere que es el formato vigente 2026, mientras que el de Humberto es una entrega real de agosto 2025. Comparado contra `draw_portada()` y `draw_cedula_evaluacion_blanco()`, encontró 2 huecos reales más, ya corregidos: Portada le faltaban los renglones "Fecha:" y "Lote:" (y "Evaluadora:" pasó a "Evaluador:", como dice esta plantilla — la Cédula sí sigue diciendo "Evaluadora:", son inconsistentes entre sí en el propio original, así que cada página replica exactamente lo que dice su propia página fuente); Cédula de Evaluación le faltaba el campo "Incidencias:" (ahora son 5 campos de RESULTADO, no 4) y todo un bloque de acuse al final ("RECIBÍ COPIA DE LA CÉDULA DE EVALUACIÓN (ACUSE)" con SÍ/NO y firma del usuario) que no existía. Verificado que la Cédula sigue cabiendo en 1 sola página tras agregar todo esto (achicando el espaciado, igual que el original).

**⚠️ Pregunta abierta para Diego, sin resolver (encontrada 15 sep):** el índice de esta plantilla 2026 es distinto al de Humberto — solo lista 3 secciones (no menciona "4. Anexos" como ítem, aunque el separador "4. ANEXOS" sí existe como página aparte más adelante en el mismo archivo), y el punto 1 incluye "Tríptico de Derechos y Obligaciones" como parte de "1. Datos del candidato" — mientras que en el expediente real de Humberto, el "Acuse de Recibido - Tríptico" físicamente aparece hasta la sección 4 (Anexos), que es donde `assemble_expediente.py` lo pone hoy. No se tocó `draw_indice()` ni el orden de `ordered_files` por esta discrepancia — falta que Diego confirme cuál de los dos formatos seguir (el realmente entregado y aceptado por Humberto, o el que parece ser la plantilla vigente 2026) antes de mover el Tríptico o simplificar el índice.
...
- Cédula de Evaluación y el IEC los llena el evaluador después de revisar el video — el script los inserta en blanco (o debería, ver hallazgo de arriba); falta un flujo para que el evaluador los llene digitalmente (ver backlog #7).
