...
| Documentos de Sesión — Ficha de Registro | 🔶 Parcial | Las páginas del expediente de Humberto para este documento específico (con datos reales del paciente, no del candidato) no se lograron ubicar como páginas sueltas dentro de las 146 del PDF de referencia — la fuente de verdad usada en su lugar fue el contenido EXACTO que exige la fila 14 (PRODUCTOS, Elemento 2) del Plan de Evaluación, ya verificado ahí. Se agregó "Folio" (reusa `expedienteNo`). **Sigue faltando:** datos del médico tratante/profesional de la salud, información toxicológica, y resumen de resultados de laboratorio/gabinete — ninguno de los 3 se captura hoy en el wizard, no es solo un tema del PDF. |
| Documentos de Sesión — Carta de Consentimiento | 🔶 Parcial | Misma fuente de verdad: fila 24 (PRODUCTOS, Elemento 3) del Plan de Evaluación. Se agregaron Edad/Fecha de nacimiento/Domicilio/Familiar a avisar (ya se capturaban en el paso "ficha", solo no se incluían aquí) y el párrafo de Aviso de Privacidad. **Sigue faltando (requiere campos nuevos en el wizard):** puntos y zonas del cuerpo que se tocarán, vestimenta recomendada, limitantes de aplicación del servicio, condiciones de preparación que debe cubrir el usuario, número de sesiones y duración, objetivos por sesión — estos últimos 3 ya existen como campos pero viven en Plan de Sesión, no en Consentimiento (el original los pide en ambos). |
| Documentos de Sesión — Plan de Sesión | ❌ Sin validar | Fuente de verdad: fila 29 (PRODUCTOS, Elemento 4) del Plan de Evaluación — pide contenido MUY distinto al que genera hoy `generatePlanSesionPDF()` (hoy: condiciones de preparación/No. de sesiones/objetivos — eso es lo que el original pide en Consentimiento y Plan de Seguimiento, no aquí). El original pide: que se integre al Plan de Seguimiento, y por cada sesión el número/fecha/hora de inicio y término/signos vitales, descripción de actividades, notas de evolución y pronóstico, y tareas para hacer en casa. Necesita una revisión de fondo (posible captura por sesión, no solo un plan general) antes de tocar el PDF — no se tocó nada todavía. |
| Documentos de Sesión — Plan de Seguimiento | ✅ Parece alineado, sin cambios necesarios | Fuente de verdad: fila 28 (PRODUCTOS, Elemento 4). Lo que genera hoy `generatePlanSeguimientoPDF()` (fechas/horarios, medio de contacto, sesiones programadas con frecuencia/duración, nota de evolución) ya cubre razonablemente los puntos de esa fila — no se le hicieron cambios, pero tampoco se comparó imagen por imagen contra un original real (no localizado). |
