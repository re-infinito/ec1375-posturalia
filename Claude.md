...
- **⚠️ Revisado el 17 sep a las 9:30 (hora de México): la fila todavía NO estaba limpia y se estaba usando** (`updated_at` de esa mañana). Trae mezcla de pruebas viejas con datos reales:
  - `autodiagnostico_data`: la firma principal es `signatureMode: 'typed'` con el nombre **"Candidato de Prueba (Admin)"** (el placeholder viejo del bypass). `'typed'` no es un modo válido (solo existen `draw` y `type`), pero contaba como firmado. También trae 142 respuestas en SÍ, el correo `diegogarzamx@outlook.com` en datos personales y **ningún documento de Registro subido** (`documentosNextcloud` vacío), por eso Alineación decía "Faltan documentos de tu Registro por subir".
  - `plan_evaluacion_data`: sus 2 PDF apuntan a la carpeta **`Portafolios/Candidato_de_Prueba_Admin_XAXX010101HNEXXXA4/02-Alineacion/`** del NAS, no a la de Diego.
  - `documentos_sesion_data`: 4 PDF subidos a la carpeta de Diego (paciente de prueba). `encuesta_data`: 8 respuestas, del formato viejo de 8 preguntas.
  - Desde el 17 sep la firma `'typed'` se trata como **sin firma** (el Autodiagnóstico te lleva al paso "Firma") y los documentos viejos se resuben al abrir cada página. Aun así, **para la prueba definitiva hay que hacer el paso 1 de abajo**, porque las respuestas y los datos de prueba seguirían en el expediente.
...
tutoriales/               13 videos mudos (<id>.mp4, ~12 MB en total) + su portada (<id>.jpg). La lista vive en crm-shell.js
                           (CrmShell.TUTORIALES). Se regrabaron con _internal_no_publicar/tutoriales-grabador/ (ver Cambios recientes)
...
documentos-nas.js         Subida de los PDF al NAS con control de versión (17 sep): huella de los datos, resubida al
                           cambiar, estados por documento — ver "Resubida por versión". Pruebas: tests/documentos-nas.test.js.
...
formatos-consultorio.js   Formatos en blanco del toolkit (Aviso, Consentimiento, Plan de Sesión, Plan de Seguimiento) como
                           bloques → PDF (jsPDF) y Word (docx); fuente única del texto del Aviso de Privacidad (también lo usa
                           documentos-sesion.html). Pruebas: tests/formatos-consultorio.test.js.
...
### Resubida por versión (`documentos-nas.js`, 17 sep)

Pedido de Diego: "si se hace algún ajuste a cualquier documento previo, debe de resubirse la versión actualizada". Antes cada página subía cada PDF **una sola vez** (`if (documentosNextcloud[key]) continue`), así que una corrección posterior nunca llegaba al NAS y el expediente se armaba con la versión vieja.

- **Huella:** cada página calcula `DocumentosNas.huella(...)` (FNV-1a sobre el JSON) de **todo lo que entra a sus PDF**, incluidos datos que vienen de otra página (nombre y firma del candidato del Autodiagnóstico) y una constante de versión de plantilla (`VERSION_DOCS_REGISTRO`, `VERSION_DOCS_ALINEACION`, `VERSION_DOCS_SESION`, `VERSION_DOC_ENCUESTA`). Junto a la ruta se guarda `documentosHuella[clave]` en el mismo JSONB. **Si cambias el formato de un PDF, sube su constante**: así se resube a todos los candidatos la próxima vez que abran la página.
- **Cuándo resube:** (a) al abrir la pantalla de resultado; (b) al cargar la página, si ya había documentos y la huella cambió; (c) 8 s después del último guardado con cambios. En (b) y (c) solo si los documentos ya se habían generado alguna vez (`yaGenerado`) y la página sigue completa. Se sobrescribe **el mismo archivo** (mismo nombre, WebDAV PUT). Lo subido antes de este cambio no tiene huella, así que **se resube una vez**.
- **Pantalla sin bloqueo:** el resultado se pinta de inmediato y la subida corre en segundo plano, con estados por documento: ⏳ Subiendo/Actualizando…, ✅ Subido, ⚠️ Cambios sin subir (el NAS conserva la versión anterior; botón "Subir versión actualizada"), 📥 Descargado, ❌ No subido. Una sola subida por página a la vez; si llega otra mientras sube, se repite al final con la huella más nueva.
- **Gates sin cambio:** `documentosFaseCompletos()` sigue viendo la ruta, así que un documento "desactualizado" no bloquea al candidato.
- **Dónde:** Autodiagnóstico (4), Plan de Evaluación (2), Documentos de Sesión (4) y Encuesta (1). `evidencias.html` no lo necesita: los archivos del candidato se suben al elegirlos. La cuenta demo no sube nada.
- **`api/subir-portafolio.js`** reintenta el PUT hasta 2 veces ante 423/429/5xx/sin respuesta (45 s por intento) y registra el status real en los logs de Vercel. El 17 sep a las 9:21 hubo 3 respuestas 502 entre 12 subidas de la misma cuenta (varias pestañas abiertas subiendo a la vez).
- Pruebas: `tests/documentos-nas.test.js` (10). Verificado en navegador con sesión y subida simuladas: resubida única de lo viejo al cargar, resubida tras editar, falla simulada → "Cambios sin subir" → reintento sube solo ese documento, y sin cambios no sube nada.
...
- **`kpi-dashboard-live.html`** — cerrado: ya no muestra datos; es un stub gateado por `Auth.renderAdminGate()` que redirige a `admin-kpis.html` tras verificar admin (no rompe links viejos).
- **`admin-utilidades.html`** — reparto de utilidades por lote de ingresos entre los 3 socios (Fernando/Lot/Diego, 16.67% cada uno por default) más un colaborador opcional "Christherapy" (50% cuando participa, deja 16.67% c/u a los socios en vez de 33.33%). Configuración por lote guardada en la tabla `reparto_utilidades`, gateado por `Auth.renderAdminGate()`.
  **Gastos fijos (18 sep):** tabla `gastos_utilidades` (SQL `2026-09-18-gastos-utilidades.sql`, ⚠️ hay que correrlo; siembra "Centro Evaluador $1,200 por certificado"). Utilidad neta del lote = ingresos cobrados − gastos, y el reparto por socio sale de la neta (nunca negativa: si los gastos superan lo cobrado, no se reparte y `neto` conserva el faltante). Cada gasto es `por_certificado` (monto × candidatos ACTIVOS del lote; desistidos/cambio de lote/administrador no generan cobro) o `fijo`; `lote` NULL = todos los lotes; el monto se edita en `admin-utilidades.html`. Fuente única: `AdminData.utilidades()`/`gastosLote()` (`admin-data.js`, con pruebas), usada por Utilidades, KPIs (`admin-kpis.html` ahora carga `admin-data.js`) y el panel del equipo. Sin la tabla, todo sigue como antes (sin gastos) y se muestra un aviso.
  **Dos tablas de costos (18 sep, `2026-09-18-gastos-para-chris.sql` ⚠️ hay que correrlo; agrega `gastos_utilidades.para` = `real` | `chris`):** `real` = lo que de verdad cobra el Centro Evaluador ($1,200/certificado) y reduce el total a repartir; `chris` = los costos con los que se calcula SU base (Centro 1 $1,250 + Centro 2 $1,250 + Certificación $1,500 = $4,000/candidato; solo en lotes con Chris). **Chris = su % de (ingresos − costos de Chris); los socios reciben el resto del neto real** (el diferencial costos-Chris − costo-real, $2,800 por certificado, se queda con ellos; sus porcentajes solo fijan cómo se dividen entre sí). Sin costos `chris` se le descuentan los reales (comportamiento anterior). Ejemplo de la hoja de Diego: $14,750 → real $13,550; Chris 50% × $10,750 = $5,375; socios $8,175 ($2,725 c/u). Lógica: `AdminData.repartir()` (pura, también la vista previa) y `fichaCandidato()` (la "Tabla por candidato" de admin-utilidades). Pruebas con los números de la hoja en `tests/admin-data.test.js`.
  **Costos por fase (18 sep, `2026-09-18-gastos-por-fase.sql` ⚠️ hay que correrlo; agrega `gastos_utilidades.fase`):** cada costo "por candidato" se genera en una fase y se cuenta por cada candidato del lote que YA PAGÓ esa fase (incluye a quien después desistió, igual que los ingresos): Centro 1 ($1,250, gastos administrativos, Chris) → Alineación; Centro 2 ($1,250, gastos administrativos, Chris) → Evaluación; Certificado ($1,500 Chris / $1,200 real) → Dictamen (fase `entrega`). `fase` NULL = cada candidato activo (comportamiento anterior). Centro 1/2 no son salida real de dinero: solo reducen la base de Chris. La pantalla muestra "Por fase — todo el lote" (pagaron/ingresos/costos por fase) y "Cómo se reparte un candidato" (la hoja fase por fase, con la participación efectiva: Chris ≈40%, cada socio ≈20% del valor real). `AdminData.porFaseLote()`; `fichaCandidato().porFase`. **Corte a hoy por candidato** (`AdminData.cortePorCandidato()`): tabla con el precio de CADA candidato (no promedio), lo recibido por fase, costos de Chris, Chris, costo real y socios, más el total a repartir hoy por persona; su suma cuadra con `utilidades()` (prueba). La tabla del candidato promedio quedó como "Referencia" plegada.
...
## Cambios recientes (18 de septiembre, 2026)

- **PWA en iPhone/iPad.** "Instalar aplicación" solo funcionaba con el evento `beforeinstallprompt`, que Safari de iOS (y cualquier navegador de iOS) **nunca dispara**, así que en iPhone no había forma de instalarla. Además `.crm-sidebtn{display:flex}` le ganaba al atributo `hidden`: el botón se veía siempre, y en iPhone al tocarlo no pasaba nada. Ahora `CrmShell._helpers.modoInstalacion(ua, maxTouchPoints, instalada, hayPrompt)` decide: `'prompt'` (Android/Chrome, aviso nativo), `'ios-safari'`/`'ios-otro'` (abre una ventana con los pasos Compartir → "Agregar a pantalla de inicio"; en Chrome de iOS o dentro de Instagram/Facebook/WhatsApp indica abrir en Safari), o `null` (ya instalada, o escritorio sin aviso → botón oculto). El iPad con iPadOS 13+ se reconoce como Mac con pantalla táctil. `crm-shell.js` también inyecta las metas `apple-mobile-web-app-*` y los íconos (`apple-touch-icon.png` 180 px, `icons/icon-192/512` y sus versiones maskable) se **redibujaron en vector** con el símbolo del logo (arco azul `#3b86f7` + barra dorada `#f9d949`, geometría medida del PNG) sobre fondo navy degradado; antes eran un marcador viejo (iOS) y el logotipo completo diminuto (Android). Para regenerarlos: `node _internal_no_publicar/01-scripts/generar_iconos.js <carpeta>` (usa el Chromium de Playwright) y copiar los PNG. Recordatorio en la ventana: la app instalada en iOS no comparte la sesión de Safari, hay que volver a iniciar sesión. `sw.js` → `paideia-app-v10`. Verificado con Playwright (iPhone Safari, iPhone Chrome, ya instalada, Android sin aviso, escritorio) y `_internal_no_publicar/tutoriales-grabador/instalar-ios.js`. Pruebas: 124.

- **Autodiagnóstico: la CURP debe estar completa.** El primer portafolio real (Diego) salió con una CURP de 17 caracteres (le faltaba el último dígito): se imprimió así en sus documentos y en el nombre de su carpeta del NAS, y la Ficha RENAP quedó sin Género ni Fecha de nacimiento, que se calculan de la CURP. La validación solo pedía que el campo no estuviera vacío. Ahora `curpValida()` exige el formato oficial de 18 caracteres (`^[A-Z]{4}\d{6}[HMX][A-Z]{5}[A-Z0-9]{2}$`), la guarda en mayúsculas y sin espacios, avisa mientras se escribe ("la que escribiste tiene N") y no deja avanzar de Datos personales. Un candidato con la CURP incompleta ya guardada vuelve a ese paso con "Continuar donde me quedé"; al corregirla sus documentos se regeneran solos (en una carpeta nueva del NAS). Verificado con Playwright.
...
- **Navegación entre secciones y declaraciones del candidato** (plan `docs/superpowers/plans/2026-09-17-navegacion-y-declaraciones.md`, spec `…/specs/2026-09-17-recursos-navegacion-declaraciones-design.md`, partes A y D; B tutoriales y C toolkit abajo):
  - **Autodiagnóstico:** selector "Ir a otra sección" arriba de cada sección y tarjeta "Regresar a una sección" en el Resultado (`seccionMaximaPermitida`/`irASeccion`). Límite: la primera sección incompleta; sin los 142 reactivos cargados el límite se queda en el Elemento 1 (validarlos sin datos truena, ver `e4898e1`). Si un salto se rechaza, el selector regresa a la sección actual (lo mismo en Documentos de Sesión). Editar nombre o CURP desde aquí resube Registro a una carpeta nueva del NAS; Plan y Documentos de Sesión se mudan cuando se vuelven a abrir.
  - **"✏️ Editar mis respuestas"** en el resultado de Plan de Evaluación, Encuesta y Evidencias (`generated = false; render()`). `FlowStatus.renderNextStepCTA` ya no truena si el contenedor desapareció (tocar "Editar" mientras se calculan los pasos).
  - **Declaraciones (textos modelo, validar con el evaluador):**
    - Plan de Evaluación: 3 casillas obligatorias antes de "Generar" (cumple requisitos, dispone del material, sin reembolsos) → `planData.declaraciones = { version: '2026-09-17', requisitos, material, sinReembolsos }` (fecha ISO al marcar; desmarcar la borra). **No van al PDF** y se excluyen de `huellaDocsAlineacion()`, así que marcarlas no resube; la resubida automática exige solo acuerdo+firma+fecha, para que los Planes anteriores sigan resubiéndose.
    - Evidencias: "Declaración de autenticidad" obligatoria para "Confirmar Entrega" (deslinda al Centro Evaluador y a Paideia Tech de evidencias falsificadas) → `planData.declaracionAutenticidad = { version, fecha }`; aparece en el comprobante PDF interno.
    - El aviso de "no aplican reembolsos" de texto sigue en los 5 puntos de pago; la aceptación explícita queda en el Plan, antes del pago de Evaluación.
    - **Equipo:** bloque "Declaraciones aceptadas" en el detalle de `admin-candidatos.html` con la fecha de cada una o "Pendiente" (`AdminData.declaraciones(row)`; solo cuentan fechas válidas y se escapan).
    - **Límites conocidos:** se exigen al generar/confirmar, no en `flow-status.js`: quien terminó antes del 17 sep nunca las aceptó, alguien puede desmarcarlas tras generar ("Editar"), y Evidencias cuenta como hecha por los archivos aunque nunca se oprima "Confirmar Entrega". **Revisar "Declaraciones aceptadas" en admin antes de liberar Entrega.** Re-marcar reemplaza la fecha original.
  - **Cuenta demo:** los datos sembrados traen las 4 declaraciones, pero un navegador sembrado antes de este cambio las verá sin marcar hasta oprimir **"Reiniciar demo"** una vez.
  - Pruebas: 81 (`tests/admin-data.test.js` +3, `tests/flow-status.test.js` +1). Verificado en navegador con copias locales y sesión simulada: saltos y bloqueo en Autodiagnóstico, las 3 casillas del Plan (fechas, huella sin cambio, 0 resubidas), Editar en las 3 páginas, declaración en el comprobante (1 página, antes de la firma), detalle de admin con fechas y "Pendiente", 0 errores en la carrera de "Editar" y 375 px sin desborde.
- **Recursos: tutoriales de la plataforma** (parte B del spec; Diego eligió "animaciones mudas" grabadas por Claude): **13 videos** — Iniciar sesión, Tu panel, Autodiagnóstico, Reforzamiento, Alineación, Biblioteca, Plan de Evaluación, Documentos de Sesión, Práctica, Examen, Encuesta, Evidencias y Entrega — de 35 a 89 s, sin audio, con subtítulos, cursor visible, clics marcados y recuadros sobre lo que se explica. Datos de la candidata ficticia "Ana Sofía Demo Ramírez" (`ana.demo@ejemplo.com`); ninguna contraseña real.
  - **Dónde se ven:** sección "Tutoriales de la plataforma" arriba en `recursos.html` (siempre disponible, sin candado) y botón **"▶ Ver cómo se hace"** en el encabezado de cada paso (`crm-shell.js`, según `currentPageId`; en `estudio.html`, que usa el rail, es `#btn-tut` en su barra superior). Se abren en una ventana (`CrmShell.abrirTutorial(id, disparador)`): Esc, el fondo o ✕ la cierran y el foco regresa al botón. El Guion Maestro comparte id con el Plan de Evaluación, así que ahí no aparece el botón.
  - **La lista vive en `crm-shell.js`** (`TUTORIALES`: id, título, descripción, páginas y duración en segundos); `recursos.html` la lee de ahí. `sw.js` deja pasar `/tutoriales/` directo a la red (los videos llegan como respuestas parciales 206, que `cache.put` rechaza) → `paideia-app-v8`.
  - **Cómo se grabaron / cómo regrabar** cuando cambie la interfaz: `_internal_no_publicar/tutoriales-grabador/` (gitignored). Con el servidor local de `.claude/launch.json` (puerto 8420) corriendo: `node grabar.js [id …]` → `salida/<id>.mp4` y `.jpg`; copiarlos a `tutoriales/` y actualizar `dur` en `TUTORIALES`. Usa Playwright (`playwright-core` 1.60 de `~/.npm/_npx/e41f203b7505f1fb/`, Chromium 1223 ya instalado) y ffmpeg (H.264 CRF 33 `-tune stillimage`: la mitad de peso que CRF 26 sin diferencia visible en texto). **Qué simula** (`stub.js`, inyectado tras `auth.js`): sesión, fases pagadas, subidas al NAS (`{success, path}`) y la API de sesiones de Alineación; el contenido protegido sale del paquete local `_internal_no_publicar/contenido/`; sin marca de agua (`protect.js` vacío) y `Math.random` con semilla en Práctica/Examen (la variante de la pregunta es aleatoria). Los guiones están en `guiones.js`; `botones.js` verifica el botón en las 15 páginas a 1280 y 375 px; `exp.sh` lista botones y campos de una página con la sesión simulada.
  - **Verificado:** los 13 revisados cuadro por cuadro (hojas de contacto); `botones.js`: 30/30 (botón correcto o ausente, el video se reproduce, Esc cierra, 0 errores, 0 px de desborde); `recursos.html` a 375 px en 2 columnas sin desborde. Pruebas: 90 (`tests/crm-shell.test.js` +2: cada paso tiene tutorial, ids únicos, formato de duración).
  - Hallazgo al grabar (no se cambió): en Documentos de Sesión, "Continuar donde me quedé" en otro dispositivo abre "Preparación del Espacio", porque ese checklist vive solo en el navegador (a propósito). Para llegar a "Tus documentos" está el selector "Ir a otro paso".
- **Recursos: toolkit profesional** (parte C del spec `…/specs/2026-09-17-recursos-navegacion-declaraciones-design.md`, pedido de Diego para subir el valor percibido): nueva página `recursos.html`, enlazada en el sidebar del candidato como **"Tutoriales y toolkit"** (grupo Recursos). Cuatro formatos para el consultorio del candidato, cada uno en **PDF** (con sus datos, listo para imprimir) y **Word editable**: Aviso de Privacidad, Consentimiento Informado (con condiciones del servicio y el aviso completo), Plan de Sesión (signos vitales, técnica, evolución, tareas) y Plan de Seguimiento (contacto + tabla de 6 sesiones). La **Hoja terapéutica** aparece como "Próximamente": otra sesión la está terminando; cuando exista, se agrega a `FORMATOS` en `formatos-consultorio.js`.
  - **Se desbloquea al terminar el proceso** = paso Evidencias hecho según `FlowStatus.getSteps()` (misma regla que el sidebar). Admins y cuenta demo lo ven abierto. Bloqueado se ve todo (valor percibido) con los botones deshabilitados y la liga a Evidencias.
  - **Datos del responsable:** nombre/domicilio/contacto del Aviso de Privacidad de Documentos de Sesión si los capturó; si no, los del Autodiagnóstico. Siempre de `FlowStatus.getRow()`, nunca del localStorage suelto (podría ser de otra cuenta). Se pueden cambiar en la página antes de descargar (no se guardan).
  - **El aviso del toolkit no menciona la evaluación:** `avisoPrivacidadBloques(d, { evaluacion: false })` quita la finalidad de integrar la grabación al portafolio EC1375 y ajusta "¿Con quién los compartimos?" (en el consultorio ya no aplica). Documentos de Sesión sigue con el texto de siempre (default `evaluacion: true`), ahora leído del módulo: mismo texto, así que la huella no cambia y nada se resube.
  - **Sin logos de RED CONOCER/ICE México** en estos formatos: son del expediente oficial y en un formato de consultorio podrían leerse como aval institucional. Pie: "Formato elaborado conforme al Estándar de Competencia EC1375".
  - jsPDF y la librería `docx` (`cdn.jsdelivr.net/npm/docx@9.5.1`) se cargan solo al descargar. Las tablas del Word llevan anchos fijos en DXA: sin ellos Pages/Google Docs/Quick Look las colapsan a una letra por columna.
  - **Verificado:** los 8 archivos generados en el navegador (sesión simulada) y en Node con el mismo módulo, revisados página por página (PDF con pdftoppm; Word convertido con LibreOffice): ningún título queda solo al pie y el pie nunca abre hoja sola. Página en los 6 casos (bloqueado, terminado, con aviso editado, admin, demo, sin sesión) y a 375 px sin desborde; un nombre editado llega al archivo (`Aviso_de_Privacidad_Consultorio_Arbol_de_Vida.pdf`). Pruebas: 88. `sw.js` → `paideia-app-v7`.
- **Documentos de Sesión: regresar a cualquier paso** (pedido de Diego): arriba de cada paso hay un selector **"Ir a otro paso"** con los 19 pasos y "Tus documentos"; en la pantalla final, la tarjeta **"Regresar a un paso"** con los 19 como botones y "Volver al último paso (Cierre y Firma)". Solo se puede ir hasta el primer paso obligatorio que falte (`pasoMaximoPermitido()`; los de más adelante salen deshabilitados), sin contar el checklist de preparación (`prep`), que vive solo en el navegador donde se llenó. Al corregir algo, la resubida por versión sube los documentos actualizados. Verificado con copia local: salto desde la pantalla final y desde el selector, bloqueo al faltar el nombre del usuario (solo 1–5 habilitados) y 375 px sin desborde.
- **Cuarta tanda de Diego (17 sep, mediodía):**
  1. **Modo presentación ajustado y centrado** (`visor-diapositivas.css`): la diapositiva medía 100vw × 100vh, así que en pantallas que no son 16:9 se estiraba con el contenido pegado arriba, la barra lateral del shell seguía visible y el escenario se cortaba a la derecha. Ahora conserva su 16:9, ocupa lo más que cabe, queda centrada con franjas negras, se ocultan barra lateral/encabezado/marco y la navegación solo aparece al pasar el mouse. Además **el contenido de las diapositivas de dos columnas se centra en vertical** (`.cols>.stackx{align-content:center}`; antes arrancaba arriba — `.cols.c-top` sigue disponible para las que deban ir arriba) y las `pad-t` centradas sin pie ya no reservan hueco abajo. Medido en las 133 pantallas a 1024×576: descentradas 19 → 0 (las que el conteo aún marca se revisaron a ojo) y con scroll interno 1 → 0.
  2. **Sin "IMAGEN SUGERIDA"**: se quitaron las 61 leyendas (47 de `ruta-estudio:slides`, 14 de `ruta-alineacion:slides`) y los 14 pies que quedaron vacíos; la "Fuente técnica" se conserva. Paquete `20260917-1130-sin-imgsug-relacionar`.
  3. **Examen: preguntas de relacionar tal cual el documento** (`EC-1375-reactivos - actualizado.docx`). Banco de **39 → 35**: Desinfección vs. Sanitización (6 descripciones → sanitización/desinfección, antes una sola de opción múltiple), **Cuadrantes abdominales** con su imagen (9 regiones, nueva; imagen recortada **sin** la lista "1: Epigastrio…" que traía al lado y daba las respuestas) y **escala de Daniels** (6 renglones en una pregunta; antes 6 preguntas). Formato en el bloque `examen:reactivos`: `tipo:'relacionar'`, `items`, `opciones`, `correctas` (índice por renglón), `imagen` opcional (`evaluacion/…` en el bucket), `itemsConLetra`. Pantalla: un selector por renglón y un solo "Comprobar"; si falla, dice qué renglones no coinciden y al reintentar conserva los correctos. **Exámenes guardados:** cada reactivo trae `v1` (índices que ocupaba en el banco de 39) y `EstudioLogica.examen.migrar()` traduce respuestas, primer intento y posición (examen presentado sigue presentado; en curso regresa a la primera sin contestar bien); `banco: 2` marca los ya traducidos. "Para ti" de la Biblioteca también traduce antes de leer. Pruebas en `tests/estudio-logica.test.js`.
     - **Interpretaciones del documento que conviene confirmar con el evaluador:** en Desinfección vs. Sanitización la clave "e), c), b), f), d), a)" se leyó como Sanitización = e, c, b y Desinfección = f, d, a; en Cuadrantes el documento repite "Flanco derecho" dos veces — el primero se corrigió a **Flanco izquierdo** (7) según la leyenda de la imagen. **No se agregó** "Relaciona la letra con cada eje del cuerpo" (biomecánica #5): sus opciones y renglones son idénticos y la clave c), b), a) no tiene sentido sin las descripciones que faltan en el documento.
  4. **Evidencias sin duplicar**: "Evidencias Requeridas" y "Sube tus archivos" repetían los mismos 6 renglones; ahora es una sola tarjeta "Tus evidencias" con descripción, obligatorio/opcional, botón de subida y estado en cada renglón.
  `sw.js` → `paideia-app-v6`.
- **Tercera tanda de pruebas de Diego (cuenta real):**
  1. **Aviso de Privacidad en su paso** (`documentos-sesion.html`, paso `privacidad`): el guion pedía "presentar el Aviso de Privacidad" y el paso no mostraba ninguno. Ahora muestra un **aviso de privacidad simplificado** con el candidato como responsable (nombre, domicilio y contacto ARCO precargados del Autodiagnóstico y editables; el texto se actualiza al escribir) para compartir pantalla o leerlo. Menciona datos sensibles de salud, finalidades (incluida la integración de la grabación y los documentos al portafolio de evaluación EC1375), derechos ARCO y consentimiento expreso. El **mismo texto va completo en la Carta de Consentimiento** (el EC1375 pide que "contenga el Aviso de Privacidad"; antes era una sola frase) y la Ficha de Registro agrega la línea de firma de enterado. `avisoPrivacidadBloques()` es la fuente única para pantalla y PDF. **Texto modelo redactado por nosotros: conviene que lo valide el evaluador.**
  2. **Ficha de Registro con cada pregunta y su campo justo debajo**: las preguntas del guion llevan `campos` en `PASO_GUION.ficha` (datos generales bajo "Recabar la información", médico tratante y laboratorio bajo la pregunta del médico, etc.). Se agregaron 3 campos que el guion pregunta y la ficha no tenía: **enfermedades crónicas o degenerativas, alergias y hábitos de alimentación y sueño** (el criterio de desempeño los enumera), también en la tabla de Antecedentes del PDF y en los datos demo.
  3. **Fecha y hora "no despliega nada"**: `autodiagnostico.html` y `documentos-sesion.html` tenían `color-scheme: dark` fijo en los campos (de cuando eran solo oscuras): en tema claro el reloj de la hora salía blanco sobre blanco y el selector se abría en oscuro. Se quitó (lo hereda de `html[data-theme]`), y `crm-shell.js` abre el calendario/reloj con `showPicker()` al tocar **cualquier parte** del campo, en todas las páginas con shell (en Chrome de escritorio solo abría el iconito).
  4. **"Continuar donde me quedé"**: además de la carga de reactivos (punto de arriba), el botón muestra "Abriendo tu avance…", un error al abrir el paso se ve en pantalla en vez de perderse en consola, y el resultado **ya no espera a subir los 4 PDF** (antes, con el NAS lento, no pintaba nada hasta terminar). Una firma guardada con modo inválido (`'typed'`) se trata como no firmada.
  5. **Resubir la versión actualizada al NAS** — ver "Resubida por versión" en "Documentos → Nextcloud".
  Verificado en navegador con copias locales de las páginas (sesión y subida simuladas, datos ficticios): pasos de Aviso y Ficha en escritorio y 375 px (0 px de desborde), PDF de Ficha y Carta revisados (ningún título del aviso queda solo al pie), selector de hora abierto con clic real, tema claro/oscuro. `sw.js` → `paideia-app-v5`.
